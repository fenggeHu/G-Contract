...
| `resources[]` | {id,max,regenPerSec?} | No | Resource pools (e.g. mana/energy); `ability.cost.resource` references their `id`. **Consumed at runtime**: spent on cast (insufficient → cast rejected), regenerated by `regenPerSec`, persisted in `player.resources` (save-schema) |
