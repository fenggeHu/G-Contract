...
| `vendor_buy` | `{npc, item, count, price, currency, required_rep?, capacity?, idempotency_key?}` | `{ok, revision, currency, items?}` or `{ok:false, err}` | **Server-authoritative** purchase: validates reputation + currency against the caller's save, mutates `player.items` under CAS; `err ∈ missing_currency/resource/insufficient_rep/capacity` |
| `vendor_sell` | `{npc, item, count, price, currency, capacity?, idempotency_key?}` | `{ok, revision, currency, items?}` or `{ok:false, err}` | **Server-authoritative** sale: removes the item and credits the currency under CAS |
| `progression_unlock` | `{tree_id, tree:{points,nodes,tiers}, node, idempotency_key?}` | `{ok, revision, progression, abilities?}` or `{ok:false, err}` | **Server-authoritative** talent unlock: validates requires/points against the tree projection, grants the node ability, writes under CAS |
| `progression_reset` | `{tree_id, idempotency_key?}` | `{ok, revision, progression, abilities?}` | **Server-authoritative** tree reset: refunds points and revokes only that tree's abilities under CAS |

> **Authoritative economy (loot/vendor/progression)**: these RPCs keep the server free of content values — the client sends the relevant **content projection** (`table`/`tree`/`price`/`currency`) and the server validates intent + mutates the caller's cloud save under **CAS** with an optional **idempotency key** (repeated calls with the same key return the first result). Offline clients fall back to local authority; see [save-schema.md](save-schema.md) §12.
