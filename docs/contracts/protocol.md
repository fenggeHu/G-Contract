...
- `combat` (optional, generic combat projection): `{ abilities: { <abilityId>: { cooldownMs, range, damage, cooldownGroup? } } }`. Produced by content (rules/values are in the content layer); the server uses it to **authoritatively resolve** `cast`, contains no gameplay values and no per-content code. Clients can use `NetClient.build_combat_data()` to generate it from Defs.
